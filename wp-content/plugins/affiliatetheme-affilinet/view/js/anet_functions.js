...
jQuery(document).ready(function() {
     ...
     * Stuff
     */
    jQuery('#affilinet_shop').on('change', function() {
        getCategoryList();
    });
...
var checkConnection = function() {
    ...
    jQuery('#search-link').attr('disabled', true).append(' <i class="fa fa-circle-o-notch fa-spin"></i>').after(' <small class="status-after" style="margin: 5px;display: inline-block;">' + anet_vars.connection + '</small>');
    ...
    jQuery.ajax({
        ...
        success: function(data){
            if(!data['message']) {
                ...
                jQuery('select#affilinet_shop').html(html);
                getCategoryList();

                resultContainer.fadeOut('fast', function() {
                    resultContainer.append('<div class="updated"><p>' + anet_vars.connection_ok + '</p></div>');
                ...
                resultContainer.fadeOut('fast', function() {
                    resultContainer.append('<div class="error"><p>' + anet_vars.connection_error + ' <i>'+data['message']+'</i></p></div>');
                    resultContainer.fadeIn('fast');
                });
...
var searchAction = function() {
    ...
    var shopId = jQuery('#at-import-window #affilinet_shop').val();
    ...
    var action = 'at_affilinet_search';

    if(ean.length > 5) {
        action = 'at_affilinet_search_ean';
    }
                ...
                if(data['items']) {
                    for (var x in data['items']) {
                        ...
                        if(data['items'][x].image !="assets/images/no.gif") {
                            ...
                            html += '<td class="image">' + anet_vars.no_image + '</td>';
                        }
                        ...
                        if(data['items'][x].exists != "false") {
                            html += '<td class="action"><a href="' + jQuery('#at-import-page').attr('data-url') + 'post.php?post=' + data['items'][x].exists + '&action=edit" target="_blanbk" title="' + anet_vars.edit + '"><i class="fa fa-edit"></i></a></td>';
                        } else {
                            html += '<td class="action"><a href="' + ajaxurl + '?action=affilinet_api_lookup&id=' + data['items'][x].id + '&height=700&width=820" class="thickbox" title="' + anet_vars.import + '"><i class="fa fa-plus-circle"></i></a> <a href="#" title="Quickimport" class="quick-import" data-id="'+data['items'][x].id+'"><i class="fa fa-bolt"></i></a></td>';
                        }
                        html += '</tr>';
                    ...
                    html += '<td colspan="8">' + anet_vars.no_products_found + '</td>';
                    html += '</tr>';
                    resultContainer.append(html);
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            if(data['rmessage']['success'] == "false") {
                ...
                jQuery(target).hide();
                jQuery(target).after('<a class="button button-primary" href="' + jQuery('#at-import-page').attr('data-url') + 'post.php?post='+data['rmessage']['post_id']+'&action=edit"><i class="fa fa-pencil"></i> ' + anet_vars.edit_product + '</a>');
...
var quickImportAction = function(id, mass, i, max_items) {
    ...
    var data = {action : 'affilinet_api_import', id : id, func : 'quick-import', '_wpnonce' : nonce};
    var tax_data = {};
...
var massImportAction = function(target) {
    ...
    jQuery(ajax_loader).addClass('active').find('p').html(anet_vars.import_count + ' ' + max_items);
...
var getCategoryList = function() {
    var shop = jQuery('#affilinet_shop').val();

    if(shop == null || shop.length == 0) {
        return;
    }

    // check if more shops are marked so deactivate this select box
    if (shop.length > 1) {
        jQuery('#at-import-window select#category').find('option:selected').removeAttr("selected");
        jQuery('#at-import-window #category-selector').hide();
        return;
    } else {
        jQuery('#at-import-window #category-selector').show();
    }
    ...
    jQuery.ajax({
        ...
        dataType: 'json',
        type: 'GET',
        data: "action=at_affilinet_categorylist&shop=" + shop,
        success: function(data){
            var html = '';
...
var checkAdblock = function() {
    setTimeout(function() {
        ...
        if(!ad
            ...
            || ad.clientHeight === 0) {
            jQuery('#checkConnection').append('<div class="alert alert-danger">' + anet_vars.adblocker_hint + '</div>');
        } else {
