...
var checkConnection = function() {
    ...
    jQuery('#category-selector').hide();

    jQuery('#search-link').attr('disabled', true).append(' <i class="fa fa-circle-o-notch fa-spin"></i>').after(' <small class="status-after" style="margin: 5px;display: inline-block;">' + tradedoubler_vars.connection + '</small>');
    ...
    jQuery.ajax({
        ...
        dataType: 'json',
        type: 'GET',
        data: "action=at_tradedoubler_shoplist",
        success: function(data){
            if(!data['message']) {
                ...
                jQuery('select#tradedoubler_shop').html(html);
                resultContainer.fadeOut('fast', function() {
                    resultContainer.append('<div class="updated"><p>' + tradedoubler_vars.connection_ok + '</p></div>');
                ...
                resultContainer.fadeOut('fast', function() {
                    resultContainer.append('<div class="error"><p>' + tradedoubler_vars.connection_error + ' <i>'+data['message']+'</i></p></div>');
                    resultContainer.fadeIn('fast');
                });
...
var searchAction = function() {
    ...
    var shopId = jQuery('#at-import-window #tradedoubler_shop').val();
    ...
    var action = 'at_tradedoubler_search';

    if(ean.length > 5) {
        action = 'at_tradedoubler_search_ean';
    }
                ...
                if(data['items']) {
                    for (var x in data['items']) {
                        ...
                        if(data['items'][x].image !="assets/images/no.gif") {
                            ...
                            html += '<td class="image">' + tradedoubler_vars.no_image + '</td>';
                        }
                        html += '<td class="title"><a href="'+data['items'][x].url+'" target="_blank">'+data['items'][x].name+'</a></td>';
                        html += '<td class="shop">'+data['items'][x].shopname+'</td>';
                        html += '<td class="price">'+data['items'][x].price+'</td>';
                        html += '<td class="category">'+data['items'][x].category+'</td>';
                        if(data['items'][x].exists != "false") {
                            html += '<td class="action"><a href="' + jQuery('#at-import-page').attr('data-url') + 'post.php?post=' + data['items'][x].exists + '&action=edit" target="_blanbk" title="' + tradedoubler_vars.edit + '"><i class="fa fa-edit"></i></a></td>';
                        } else {
                            html += '<td class="action"><a href="' + ajaxurl + '?action=tradedoubler_api_lookup&id=' + data['items'][x].id +'&height=700&width=820" class="thickbox" title="' + tradedoubler_vars.import + '"><i class="fa fa-plus-circle"></i></a> <a href="#" title="Quickimport" class="quick-import" id="'+data['items'][x].id+'"><i class="fa fa-bolt"></i></a></td>';
                        }
                        html += '</tr>';
                    ...
                    html += '<td colspan="8">' + tradedoubler_vars.no_products_found + '</td>';
                    html += '</tr>';
                    resultContainer.append(html);
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            if(data['rmessage']['success'] == "false") {
                ...
                jQuery(target).hide();
                jQuery(target).after('<a class="button button-primary" href="' + jQuery('#at-import-page').attr('data-url') + 'post.php?post='+data['rmessage']['post_id']+'&action=edit"><i class="fa fa-pencil"></i> ' + tradedoubler_vars.edit_product + '</a>');
                ...
                jQuery('body table.products tr[id='+id+'] .action i').removeClass('fa-plus-circle').addClass('fa fa-edit').closest('a').removeClass('thickbox').attr('target', '_blank').attr('href', jQuery('#at-import-page').attr('data-url') + 'post.php?post='+data['rmessage']['post_id']+'&action=edit');
            }
        }
    });
};
...
var quickImportAction = function(id, mass, i, max_items) {
 ...
 //   console.log(shopid);
 //   console.log("v1");
    var target = jQuery('#results .item[id='+id+']').find(".action a.quick-import");
    var ajax_loader = jQuery('.at-ajax-loader');

    var nonce = jQuery('#at-import-page').attr('data-import-nonce');
    var data = {action : 'tradedoubler_api_import', id : id, func : 'quick-import', '_wpnonce' : nonce};
    var tax_data = {};
...
var massImportAction = function(target) {
    ...
    jQuery(ajax_loader).find('.progress-bar').attr('data-item',1);
    jQuery(ajax_loader).find('.progress-bar').css('width', '0%').html('0%');
    jQuery(ajax_loader).addClass('active').find('p').html(tradedoubler_vars.import_count + ' ' + max_items);
...
var getCategoryList = function() {
    ...
    jQuery.ajax({
        ...
        dataType: 'json',
        type: 'GET',
        data: "action=at_tradedoubler_categorylist",
        success: function(data){
            var html = '<option value="">-</option>';
            ...
            jQuery('#at-import-window select#category').html(html);
        }
    });
}
...
var checkAdblock = function() {
    setTimeout(function() {
        ...
        if(!ad
            ...
            || ad.clientHeight === 0) {
            jQuery('#checkConnection').append('<div class="alert alert-danger">' + tradedoubler_vars.adblocker_hint + '</div>');
        } else {
