	...
	<div class="table-responsive">
		<table class="table table-hover table-product table-striped table-product-y">
			<colgroup>
				...
				if(apply_filters('at_table_y_hide_price', true)) { echo '<col>'; } ?>
				<col>
			</colgroup>
			<thead>
				<tr class="product-row-header">
					<th></th>
					<th></th>
					<th><?php echo apply_filters('at_table_y_product_name_row_title', __('Modell', 'affiliatetheme')); ?></th>
					<?php
					if(apply_filters('at_table_y_hide_review_summary', true) && $product_review_hide != 1) {
						?><th><?php echo apply_filters('at_table_y_product_review_row_title', __('Testergebnis', 'affiliatetheme')); ?></th><?php
					}
					...
					if('1' != get_field('product_price_hide', 'option') && apply_filters('at_table_y_hide_price', true)) {
						?>
						<th><?php echo apply_filters('at_table_y_product_price_row_title', __('Preis', 'affiliatetheme')); ?></th>
						<?php
					}
					?>
					<th></th>
				...
				foreach($products as $product) { 
					...
					<tr class="product-<?php echo $product->ID; ?>">
						...
						if (apply_filters('at_table_y_show_number_row', true)) { ?>
							<td class="<?php echo(in_array($product->ID, $custom_product_table_highlight) ? 'table-highlight ' : ''); ?>table-counter">
