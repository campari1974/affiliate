...
if(has_post_thumbnail()) {
	$thumbnail = get_the_post_thumbnail($post->ID, 'product_gallery', array('class' => 'img-responsive'));
	$thumbnail_tiny = get_the_post_thumbnail($post->ID, 'product_tiny', array('class' => 'img-responsive'));
} else {
	$thumbnail = at_post_thumbnail($post->ID, 'product_gallery');
}
...
if($gallery_external) {
	...
	foreach($gallery_external as $k => $item) {
		...
		if($dsgvo_external_images_proxy) {
		    $url = at_external_images_get_url($post->ID, $k+1);
        } else {
			$url = $item['url'];
        }
		...
		<div class="carousel-inner cont-slider">
				...
				<div class="item active">
					<div class="img-wrapper">
						...
						if('1' == $product_image_link) {
							...
							echo '<a class="img-wrapper-inner" href="' . get_product_link($post->ID) . '" title="' . get_the_title() . '" target="' . $product_external_target . '" rel="nofollow">';
						} else {
			...
			if($gallery) {
				foreach($gallery as $item) { ?>
					<div class="item <?php if(!$thumbnail && $i==0) echo 'active'; ?>">
						<div class="img-wrapper">
							<?php
							if('1' == $product_image_link) {
								$product_external_target = (get_field('product_external_target', 'option') ? get_field('product_external_target', 'option') : '_blank');
		...
		<ol class="carousel-indicators">
			...
			if($thumbnail_tiny) { ?>
				<li class="active" data-slide-to="0 " data-target="#productGallery">
					<?php echo $thumbnail_tiny; ?>
				</li>
			<?php $i++; } ?>
			
			<?php if($gallery) { foreach($gallery as $item) { ?>
				<li class="<?php if(!$thumbnail_tiny && $i==0) echo 'active'; ?>" data-slide-to="<?php echo $i; ?>" data-target="#productGallery">
