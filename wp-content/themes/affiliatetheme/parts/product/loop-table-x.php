<?php
global $products, $product_table_highlight, $product_details_tax_hide, $product_details_fields_hide, $product_rating_hide, $product_review_hide, $product_button_detail_hide, $product_button_buy_hide, $product_field_groups
;
...
if($products) {
	...
	<div class="table-responsive">	
		<table class="table table-striped table-hover table-product table-product-x">
			<colgroup>
				<col>
				<?php foreach($products as $product) { echo '<col style="width:' . $col_width . '%;">'; } ?>
			</colgroup>
			<thead>
			...
			<tbody>
				...
				<tr class="product-row-title">
					<td><?php echo apply_filters('at_table_x_product_name_row_title', __('Modell', 'affiliatetheme')); ?></td>
					<?php
                    foreach($products as $product) {
                ...
                if($fields && count($fields) > 1 && get_product_price($product->ID, 0, true) && apply_filters('at_table_x_hide_first_price', true)) { ?>
                    <tr class="product-row-price-1">
                        <td><?php echo apply_filters('at_table_x_product_price_row_title', __('Preis', 'affiliatetheme')); ?></td>
                        <?php
                        foreach($products as $product) {
                ...
                if(apply_filters('at_table_x_hide_review_summary', true) && $product_review_hide != 1) {
                    ...
                    <tr class="product-row-review-summary">
                        <td><?php echo apply_filters('at_table_x_product_review_row_title', __('Testergebnis', 'affiliatetheme')); ?></td>
                        <?php foreach ($products as $product) { ?>
                            <td class="product-review-summary <?php echo(in_array($product->ID, $custom_product_table_highlight) ? 'table-highlight ' : ''); ?> product-<?php echo $product->ID; ?>">
                                <?php
                ...
                if(apply_filters('at_table_x_hide_rating', true) && $product_rating_hide != 1) { ?>
                    <tr class="product-row-rating">
                        <td><?php echo apply_filters('at_table_x_product_rating_row_title', __('Bewertung', 'affiliatetheme')); ?></td>

                        <?php foreach($products as $product) { ?>
                            <td class="product-rating <?php echo (in_array($product->ID, $custom_product_table_highlight) ? 'table-highlight ' : ''); ?> product-<?php echo $product->ID; ?>">
                                <?php
