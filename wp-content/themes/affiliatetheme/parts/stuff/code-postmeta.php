<p class="post-meta">
	<?php if(get_field('blog_single_meta_published', 'option') != '1') { ?>
		<span class="post-meta-author">
			<?php echo __('Veröffentlicht von', 'affiliatetheme'); ?> <a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><?php echo esc_attr(get_the_author()); ?></a>
		</span>
	<?php }
	...
	if(get_field('blog_single_meta_comments', 'option') != '1') {
		if(comments_open()) {
			...
			comments_number(__('Keine Kommentare', 'affiliatetheme'), __('1 Kommentar', 'affiliatetheme'), __('% Kommentare', 'affiliatetheme')); echo '</a></span>';
		}
	}
	?>
</p>