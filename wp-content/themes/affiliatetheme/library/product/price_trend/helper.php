...
if ( ! function_exists( 'at_price_trend_render_button' ) ) {
    ...
    function at_price_trend_render_button($post_id) {
        if (at_show_price_trend($post_id, 'bottom')) {
            echo '<a href="#price-trend" class="btn btn-sm btn-link smoothscroll" data-offset="100"><i class="fa fa-line-chart"></i> ' . __('Preisverlauf', 'affiliatetheme') . '</a>';
        }

        if (at_show_price_trend($post_id, 'tab')) {
            $hide_tabs = get_field('product_single_tabs_remove', 'option');
            if ($hide_tabs == '1') {
                echo '<a href="#price_trend" class="btn btn-sm btn-link"><i class="fa fa-line-chart"></i> ' . __('Preisverlauf', 'affiliatetheme') . '</a>';
            } else {
                echo '<a href="#" class="btn btn-sm btn-link open-tab" data-target="tab-price_trend"><i class="fa fa-line-chart"></i> ' . __('Preisverlauf', 'affiliatetheme') . '</a>';
            }
        }

        if (at_show_price_trend($post_id, 'modal')) {
            echo '<a href="#" class="btn btn-sm btn-link" data-toggle="modal" data-target="#priceTrend"><i class="fa fa-line-chart"></i> ' . __('Preisverlauf', 'affiliatetheme') . '</a>';
        }
    }
}