...
function endcore_products_shortcode($atts, $content = null) {
	...
	} else if($layout == "grid" || $layout == "grid-hover") {
		/*
		 * Layout: grid or grid-hover
		 */
		$products_grid = new WP_Query($args); if ($products_grid->have_posts()) :
			$i=0;
			$rand = rand(0,13337);
			...
			if('true' == $slider) {
				...
				<div id="carousel-' . $rand . '" class="carousel slide" data-interval="' . $interval . '" data-ride="carousel">
					<div class="controls pull-right hidden-xs">
	                    <a class="left fa fa-chevron-left btn btn-link" href="#carousel-' . $rand . '" data-slide="prev"></a>
	                    <a class="right fa fa-chevron-right btn btn-link" href="#carousel-' . $rand . '" data-slide="next"></a>
	                </div>
...
function endcore_top_rated_shortcode($atts, $content = null) {
	...
	if($products) {
		...
		if($product_items) {
			...
			ob_start();
			get_template_part('parts/product/code', 'top-rated');
			$output .= ob_get_contents();
			ob_end_clean();
		}
	}
