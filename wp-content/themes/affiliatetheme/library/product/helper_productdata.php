...
if ( ! function_exists('at_product_data_import_form')) {
    function at_product_data_import_form()
    {
        ...
        $url = $_POST['url'];
        /*if (preg_match('/^https:\/\/www\.billiger\.de\/products\//', $url)) {
            $data = at_get_billiger_product_sheet($url);
        } else if (preg_match('/href="https:\/\/www\.idealo\.de\/preisvergleich\/OffersOfProduct\//', $url)) {
            $data = at_get_idealo_product_sheet($url);
        } else if (preg_match('/^http:\/\/www\.preis\.de\/produkte\//', $url)) {
            $data = at_get_preisde_product_sheet($url);
        } else*/
        ...
        <form id="at_product_data_import_form">
            ...
            /*  foreach($taxonomies as $taxonomy){
                ...
                <select id="<?php echo $taxonomy ?>" name="<?php echo $taxonomy ?>">
                    <?php if (isset($data[$taxonomy]) && !empty($data[$taxonomy])) {?><option value="<?php echo $data[$taxonomy] ?>"><?php echo $data[$taxonomy] ?></option> <?php }?>
                    <option value=""></option>
                </select>
                </div>
                <?php
...
if(!function_exists('at_get_icecat_product_sheet')){
    function at_get_icecat_product_sheet($url){
        ...
        if(strpos($url, 'icecat.de') !== false) {
            $lang = 'de';
        } else if(strpos($url,'icecat.nl') !== false) {
            $lang = 'nl';
        } else if(strpos($url,'icecat.es') !== false) {
            $lang = 'es';
        } else if(strpos($url,'icecat.co.uk') !== false) {
            $lang = 'en';
        } else if(strpos($url, 'icecat.us') !== false) {
            $lang = 'en';
        } else if(strpos($url, 'icecat.be') !== false) {
            $lang = 'be';
        } else if(strpos($url, 'icecat.ua') !== false) {
            $lang = 'ua';
        } else if( strpos($url, 'icecat.at') !== false) {
            $lang = 'de';
        } else if(strpos($url, 'icecat.ch') !== false) {
            $lang = 'de';
        } else if(strpos($url, 'icecat.it') !== false) {
            $lang = 'it';
        } else {
            $lang = 'en';
        }
        ...
        foreach($productdata->data->GeneralInfo as $key=>$value){
            if(is_object($value)){
                foreach($value as $ke=>$val){
                    ...
                    } else if (is_array($val)){} else{
                        $returndata[$ke] = $val;
                    }
                }
            } else if (is_array($value)){} else{
