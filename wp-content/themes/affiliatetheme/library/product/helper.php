    ...
    function get_product_button_text($post_id, $shop_id = 0, $pos = '', $short = false) {
        ...
        if($short == true) {
            if(has_filter('at_product_api_button_short_text')) {
                $buy_button_short = apply_filters( 'at_product_api_button_short_text', $buy_button_short, $product_portal, $product_shop, $pos, $short );
            }
         ...
         */
        if($output_amazon = apply_filters( 'at_get_amazon_product_button_text', '', $product_portal, $product_shop, $short )) $buy_button = $output_amazon;
        if($output_affilinet = apply_filters( 'at_get_affilinet_product_button_text', '', $product_portal, $product_shop, $short )) $buy_button = $output_affilinet;
        if($output_zanox = apply_filters( 'at_get_zanox_product_button_text', '', $product_portal, $product_shop, $short )) $buy_button = $output_zanox;

        if(has_filter('at_product_api_button_text')) {
            $buy_button = apply_filters( 'at_product_api_button_text', $buy_button, $product_portal, $product_shop, $pos, $short );
        }
...
if ( ! function_exists( 'get_product_button' ) ) {
    ...
    function get_product_button($post_id, $shop_id, $pos, $class = '', $short = false, $force = true, $hide_var = 0) {
        ...
        if('detail' == $pos) {
            ...
            $attr = ' rel="nofollow" target="' . $product_external_target . '"';
...
if ( ! function_exists( 'get_product_link_params' ) ) {
    ...
    function get_product_link_params($post_id, $product_fakeshop) {
        ...
        if('1' == $product_fakeshop) {
            $output = 'target="' . $product_external_target . '" rel="nofollow"';
        }
        ...
        foreach($taxonomy_names as $tax) {
            if(!is_wp_error($terms = get_terms($tax, 'hide_empty=0'))) {
                ...
                <div class="form-group col-xs-6">
                    ...
                    <select class="multi" fieldname="' . $taxonomy->rewrite['slug'] . '" name="tax[' . $tax . '][]" multiple>';
                foreach($terms as $term) {
                    $selected = '';
                    if(isset($current[$tax])) {
                        if(in_array($term->slug, $current[$tax])) {
                            $selected = 'selected';
                        }
                    }
...
if ( ! function_exists( 'at_get_acf_field_groups' ) ) {
    ...
    function at_get_acf_field_groups($args = false, $post__in = array()) {
        ...
        // cache
        $found = false;
        $clear_cache = apply_filters('at_acf_field_groups_clear_cache', false);
...
if ( ! function_exists( 'sortByPrice' ) ) {
    ...
    function sortByPrice($a, $b) {
        if ($a['field_553b8257246b5'] == $b['field_553b8257246b5']) return 0;
...
if ( ! function_exists( 'get_product_review_rating' ) ) {
     ...
     * @return string
     */
    function get_product_review_rating($rating, $max, $empty, $half, $full) {
        $output = '';
...
if ( ! function_exists( 'at_get_shop_id' ) ) {
     ...
     * @return int
     */
    function at_get_shop_id($unique_id, $name = '', $create = false) {
        global $wpdb;
