 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
	    ...
	    public function __construct() {
			...
			$this->theme_customize_file 	= apply_filters('radium_theme_importer_theme_customize_file', $this->demo_files_path . $this->theme_customize_file_name);
	    ...
	    public function add_admin() {
	        ...
	        add_submenu_page('themes.php', "", "Import Demo Data", 'switch_themes', 'radium_demo_installer', array($this, 'demo_installer'));
        ...
        public function check_previous_meta( $continue, $post_id, $meta_key, $meta_value, $unique ) {
			...
			if(get_post_type() == 'sidebar' || get_post_type() == 'product' || get_post_type() == 'shop' || get_post_type() == 'page')
				return;
    	...
    	public function intro_html() {
			...
			<div class="wrap" id="xcore-page">
				<h1><?php _e('affiliatetheme.io &raquo; Demo Daten', 'affiliatetheme-backend'); ?></h1>
				<div class="content-wrapper">
					<p class="tie_message_hint"><?php _e('Mit diesem Tool kannst du per Knopfdruck die Demo Daten installieren. Wenn du den Import ausführst, geschehen folgende Dinge:', 'affiliatetheme-backend'); ?></p>

					<ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;">
						<li><?php _e('Es werden keine existierenden Seiten, Kategorien, Bilder oder andere Daten geändert oder gelöscht.', 'affiliatetheme-backend'); ?></li>
						<li><?php _e('WordPress Einstellungen werden nicht geändert.', 'affiliatetheme-backend'); ?></li>
						<li><?php _e('Es werden Beiträge, Seiten, Shops, Produkte, Bilder, Widgets und Menüs importiert.', 'affiliatetheme-backend'); ?></li>
						<li><?php _e('Es werden Bilder heruntergeladen und im Inhalt platziert, die Bilder darfst du gerne später weiter nutzen.', 'affiliatetheme-backend'); ?></li>
				 		<li><?php _e('Bitte klicke nur einmal auf <strong>Importvorgang starten</strong>, dies kann einige Minuten dauern.', 'affiliatetheme-backend'); ?></li>
					</ul>

					<h3><?php _e('Folgende Plugins sollten installiert sein:', 'affiliatetheme-backend'); ?></h3>

					<ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;">
						<li>WooSidebars (<a href="https://wordpress.org/plugins/woosidebars/" target="_blank"><?php _e('herunterladen', 'affiliatetheme-backend'); ?></a>)</li>
						<li>Contact Form 7 (<a href="https://wordpress.org/plugins/contact-form-7/" target="_blank"><?php _e('herunterladen', 'affiliatetheme-backend'); ?></a>)</li>
					</ul>

					<h3><?php _e('Sobald der Import erfolgreich war, führe bitte folgendes Einstellungen aus:', 'affiliatetheme-backend'); ?></h3>

					<ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;">
						<li><mark><?php printf(__('<a href="%s" target="_blank">Optionen</a></mark> öffnen und einmalig speichern.', 'affiliatetheme-backend'), admin_url('admin.php?page=acf-options-allgemein')); ?></li>
						<li><mark><?php printf(__('Einstellungen</mark> › <mark><a href="%s" target="_blank">Lesen</a></mark> öffnen und die Seite <strong>Startseite</strong> unter <mark>"statische Seite"</mark> als Startseite definieren.', 'affiliatetheme-backend'), admin_url('options-reading.php')) ; ?></li>
						<li><mark><?php printf(__('Einstellungen</mark> › <mark><a href="%s" target="_blank">Lesen</a></mark> öffnen und die Seite <strong>Blog</strong> unter <mark>"statische Seite"</mark> als Beitragsseite definieren.', 'affiliatetheme-backend'), admin_url('options-reading.php')); ?></li>
						<li><mark><?php printf(__('Design</mark> › <mark><a href="%S" target="_blank">Menüs</a></mark> öffnen und alle 3 Navigationen (Topbar, Hauptnavigation, Footer) an die entsprechenden Stellen setzen', 'affiliatetheme-backend'), admin_url('nav-menus.php')); ?></li>
					</ul>

					<div class="alert alert-info"><?php _e('Solltest du eine Fehlermeldung nach dem Import bekommen (z.B. Failed to...), versuche den Import nochmal neu zu starten.', 'affiliatetheme-backend'); ?></div>
			 <?php

			 if( !empty($this->imported_demos) ) { ?>

			  	<div class="alert alert-success">
			  		<?php _e('Demo Daten sind bereits importiert', 'affiliatetheme-backend'); ?>
			  	</div><?php
			  }
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = __('Erneut importieren', 'radium');

			} else {

				$button_text = __('Importvorgang starten', 'radium');
	     ...
	     * @return null
	     */
		public function process_imports( $content = true, $options = true, $options_2 = true, $widgets = true) {
			...
			if ( $options && !empty( $this->theme_customize_file ) && is_file( $this->theme_customize_file ) ) {
				$this->set_demo_theme_customize( $this->theme_customize_file );
			}
		...
		public function set_demo_theme_customize( $file ) {
				...
				if ( file_exists( $file ) ) {
					...
					if ( !empty( $data ) || is_array( $data ) ) {
						$data = apply_filters( 'radium_theme_import_theme_customize', $data );
					...
					do_action( 'radium_importer_after_theme_customize_import', $this->active_import, $this->demo_files_path );

				} else {

					wp_die(
						__( 'Theme customize Import file could not be found. Please try again.', 'radium' ),
						'',
						array( 'back_link' => true )
					);
				}
