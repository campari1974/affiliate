...
' . $at_debug_info_plugins;
		...
		<div class="wrap" id="xcore-page">
            ...
            <?php if(@DISABLE_WP_CRON == 1) { ?>
                <div class="alert alert-danger">
                    ...
                    <p><mark>define('DISABLE_WP_CRON', 'false');</mark></p>
			...
			<table class="at_debug_table widefat" cellspacing="0" id="status">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php if ( defined('WP_DEBUG') && WP_DEBUG ) echo '<mark>' . '&#10004;' . '</mark>'; else echo '&ndash;'; ?></td>
					</tr>
					<tr>
			...
			<table class="at_debug_table widefat" cellspacing="0" id="status">
				...
				<tbody>
					...
					foreach($installed_themes as $theme) {
						...
						?>
						<tr>
							<td width="300"><?php echo $name; if($activated) echo ' (active)'; ?></td>
								...
								<tr>
									<td>
										...
										if(0 < strpos($cron, 'ffiliatetheme')) {
											...
											if('affiliatetheme_affilinet_api_update' == $cron) {
												echo ' (<a href="#" class="start-cron" data-cron="affilinet_api_update" data-hash="' . ANET_CRON_HASH.'">'.__('Cronjob jetzt ausführen', 'affiliatetheme-backend').'</a>)';
											} else if('affiliatetheme_zanox_api_update' == $cron) {
												echo ' (<a href="#" class="start-cron" data-cron="zanox_api_update" data-hash="' . ZANOX_CRON_HASH.'">'.__('Cronjob jetzt ausführen', 'affiliatetheme-backend').'</a>)';
											} else if('affiliatetheme_amazon_api_update' == $cron) {
                                            ...
                                            } else if('affiliatetheme_belboon_api_update' == $cron) {
            ...
            <div class="alert alert-info">
                <h2><?php _e('Cronjobs manuell ausführen', 'affiliatetheme-backend'); ?></h2>
                ...
                <?php
                if ( is_plugin_active( 'affiliatetheme-amazon/affiliatetheme-amazon.php' ) )  echo '<h3>' . __('Amazon API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=amazon_api_update&hash=' . AWS_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-affilinet/affiliatetheme-affilinet.php' ) ) echo '<h3>' . __('affilinet API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=affilinet_api_update&hash=' . ANET_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-zanox/affiliatetheme-zanox.php' ) )  echo '<h3>' . __('Zanox API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=zanox_api_update&hash=' . ZANOX_CRON_HASH);
				if ( is_plugin_active( 'affiliatetheme-belboon/affiliatetheme-belboon.php' ) )  echo '<h3>' . __('Belboon API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=belboon_api_update&hash=' . BBOON_CRON_HASH);
				if ( is_plugin_active( 'affiliatetheme-ebay/affiliatetheme-ebay.php' ) )  echo '<h3>' . __('eBay API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=ebay_api_update&hash=' . EBAY_CRON_HASH);
				if ( is_plugin_active( 'affiliatetheme-adcell/affiliatetheme-adcell.php' ) )  echo '<h3>' . __('Adcell API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=adcell_api_update&hash=' . ADCELL_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-cj/affiliatetheme-cj.php' ) )  echo '<h3>' . __('CJ API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=cj_api_update&hash=' . CJ_CRON_HASH);
				if ( is_plugin_active( 'affiliatetheme-rakuten/affiliatetheme-rakuten.php' ) )  echo '<h3>' . __('Rakuten API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=rakuten_api_update&hash=' . RAKUTEN_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-tradetracker/affiliatetheme-tradetracker.php' ) )  echo '<h3>' . __('Tradetracker API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=tradetracker_api_update&hash=' . TRADETRACKER_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-tradedoubler/affiliatetheme-tradedoubler.php' ) )  echo '<h3>' . __('Tradedoubler API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=tradedoubler_api_update&hash=' . TRADEDOUBLER_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-daisycon/affiliatetheme-daisycon.php' ) )  echo '<h3>' . __('Daisycon API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=daisycon_api_update&hash=' . DAISYCON_CRON_HASH);
                if ( is_plugin_active( 'affiliatetheme-financeads/affiliatetheme-financeads.php' ) )  echo '<h3>' . __('FinanceAds API', 'affiliatetheme-backend') . '</h3>' . admin_url('admin-ajax.php?action=financeads_api_update&hash=' . FINANCEADS_CRON_HASH);
                ?>
            </div>
		</div>
		...
		<script type="text/javascript">
			jQuery('.start-cron:not(.noevent)').click(function(e) {
				var btn = jQuery(this);
				jQuery(btn).append(' <i class="fa fa-circle-o-notch fa-spin"></i>').addClass('noevent');
