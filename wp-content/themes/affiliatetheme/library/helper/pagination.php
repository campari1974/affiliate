...
function pagination($range = 3, $show_one_pager = true, $show_page_hint = false)
{
	...
	if($num_of_pages > 1)
	{
		...
		{
			$output .= '<li><a href="' . get_pagenum_link(1) . '" title="'.__('Seite 1 - Neueste Artikel', 'affiliatetheme').'" data-page="1">«</a></li>';
		}
		if($show_one_pager && $current_page > 1)
		{
			$output .= '<li><a href="' . get_pagenum_link($current_page - 1) . '" title="Seite ' . ($current_page - 1) . __('- Neuere Artikel', 'affiliatetheme').'" data-page="' . ($current_page - 1) . '">‹</a></li>';
		}
		...
		for($i = 1; $i <= $num_of_pages; $i++)
		{
			if($i < $current_page + $range + 1 && $i > $current_page - $range - 1)
			{
				...
				{
					$output .= '<li><a href="' . get_pagenum_link($i) . '" title="'.__('Seite', 'affiliatetheme'). ' '.$i . '" data-page="' . ($i) . '">' . $i . '</a></li>';
				}
			}
		}
		...
		{
			$output .= '<li><a href="' . get_pagenum_link($current_page + 1) . '" title="Seite ' . ($current_page + 1) . __('- Ältere Artikel', 'affiliatetheme').'" data-page="' . ($current_page + 1) . '">›</i></a></li>';
		}
		if($current_page < $num_of_pages - 1 && $current_page + $range < $num_of_pages && $num_of_display_pages < $num_of_pages)
		{
			$output .= '<li><a href="' . get_pagenum_link($num_of_pages) . '" title="Seite ' . $num_of_pages . __('- Älteste Artikel', 'affiliatetheme').'" data-page="' . ($num_of_pages) . '">»</a></li>';
		}
